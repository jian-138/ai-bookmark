...
// �?Chrome 扩展的开发者工具控制台中运行此脚本

async function testChromeExtensionConnection() {
  console.log('=== 测试 Chrome 扩展连接 ===');
  
  // 测试 1: 检查配�?  console.log('1. 检�?API 配置...');
  try {
    // 模拟 background.js 中的配置
    const API_BASE_URL = 'https://ai-bookmark-production-5ecc.up.railway.app';
    console.log(`   API 地址: ${API_BASE_URL}`);
    console.log('�?配置检查通过');
  } catch (error) {
    console.error('�?配置检查失�?', error);
    return;
  }
  ...
  console.log('\n2. 测试网络连接...');
  try {
    const response = await fetch('https://ai-bookmark-production-5ecc.up.railway.app/', {
      method: 'GET',
      mode: 'cors',
      credentials: 'omit'
    });
  ...
  console.log('\n4. 检�?Chrome 扩展权限...');
  try {
    // 检查是否有必要的权�?    const permissions = ['storage', 'contextMenus', 'activeTab', 'scripting'];
    console.log('�?Chrome 扩展权限检查完�?);
    console.log('   需要的权限:', permissions);
    
    // 检查本地存储权�?    const testStorage = await chrome.storage.local.set({test: 'value'});
    const testRetrieval = await chrome.storage.local.get('test');
    if (testRetrieval.test === 'value') {
      console.log('�?Chrome 存储权限正常');
    }
  } catch (error) {
    console.error('�?Chrome 扩展权限检查失�?', error.message);
  }
  
  console.log('\n=== 测试完成 ===');
  console.log('如果所有测试都通过，Chrome 扩展应该可以正常登录');
  console.log('如果还有问题，请检�?');
  console.log('1. 后端服务是否正在运行');
  console.log('2. Chrome 扩展的网络权限配�?);
  console.log('3. 浏览器控制台中的详细错误信息');
}

// 运行测试
console.log('准备运行 Chrome 扩展连接测试...');
console.log('请在 Chrome 扩展的开发者工具控制台中运�? testChromeExtensionConnection()');
